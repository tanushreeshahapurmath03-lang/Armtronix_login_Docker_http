...
const UserProfile = () => {
  ...
  //           if (!checkOut && checkIn) {
  //             status = "Present";  // If only checked in, mark as "Present"
  //           } else if (checkOut && record.status === "Full Day") {
  //             status = "Full Day";
  //           } else if (checkOut && record.status === "Half Day") {
  //             status = "Half Day";
  //           }
  //         }
  //       }
  ...
  const paginatedAttendance = attendanceData.slice(
    (attendancePage - 1) * entriesPerPage,
    attendancePage * entriesPerPage
  );

  const paginatedLeave = attendance.leaveHistory.slice(
    (leavePage - 1) * entriesPerPage,
    leavePage * entriesPerPage
  );
    ...
    const fetchAttendanceHistory = async () => {
      try {
        ...
        const res = await fetch(`${backendURL}/api/user/attendance/userhistory`, {
          ...
          body: JSON.stringify({
            userId: userId,  // Filter by the logged-in user's ID
            fromDate: "2025-04-01",  // Example date range
            toDate: "2026-04-01",
          }),
        });
  ...
  // This function is updated to properly extract assignedTo IDs
  const handleEditTask = (task) => {
    // If admin - set all fields, otherwise only status and rationale
    if (isAdmin) {
      // Extract assignedTo IDs properly
      const assigneeIds = [];
      ...
      if (task.assignedTo && Array.isArray(task.assignedTo)) {
        task.assignedTo.forEach(assignee => {
          ...
          } else if (typeof assignee === 'object' && assignee.id) {
  ...
  const handleUpdateTask = async (e) => {
    ...
    try {
      ...
      const endpoint = isAdmin
        ? `/api/admin/tasks/${editingTaskId}`
        : `/api/tasks/${editingTaskId}/update-status`;

      let dataToSend;

      if (isAdmin) {
        // Make sure assignedTo contains valid IDs and is properly formatted
        dataToSend = {
          ...taskFormData,
  ...
  const handleApproveTask = async (approved) => {
    try {
      const token = localStorage.getItem('token');
      const response = await api.put(`/api/admin/tasks/${taskForApproval.id}/approve`,
        { approved },
        {
          headers: {
            Authorization: `Bearer ${token}`
      ...
      // Update task in state
      setTasks(tasks.map(task =>
        task.id === taskForApproval.id ? response.data : task
      ));
                          ...
                          <tbody>
                            {monthlyReport.map((entry, idx) => (
                              <tr key={idx}>
                                ...
                                {/* <td>{entry.status}</td> */}
                                <td className={entry.status === "Pending Checkout" ? "text-yellow-500 font-semibold" : ""}>
                                  {entry.status}
                                </td>
                    ...
                    <form onSubmit={editingTaskId ? handleUpdateTask : handleAddTask}>
                      {isAdmin && (
                        <>
                          ...
                          <div className="form-group">
                            ...
                            <p className="form-hint">Hold Ctrl/Cmd to select multiple employees</p>
                          </div>
                        </>
                      )}
                      ...
                      <div className="form-group">
                        ...
                        <textarea
                          ...
                          className="form-input"
                          rows="2"
                          placeholder={isAdmin ? "Provide reasoning or context for this task..." : "Explain your progress or any blockers..."}
                        />
                      </div>
                        ...
                        <div className="task-details">
                          ...
                          {isAdmin && task.assignedTo && (
                            <div className="task-assignees">
                              <strong>Assigned to:</strong> {Array.isArray(task.assignedTo)
